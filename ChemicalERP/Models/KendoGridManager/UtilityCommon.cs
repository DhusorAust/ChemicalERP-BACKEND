...
namespace ChemicalERP.Models.KendoGridManager
{
    public class UtilityCommon
    {
         ...
         AzFilter.GridFilter filter, List<object> parameters)
        {
            ...
            //bool isValidDate = DateTime.TryParse(filter.Value, out DateTime result);
            string[] formats = { "MM/dd/yyyy", "dd/MM/yyyy", "yyyy-MM-dd", "yyyy/MM/dd", "dd-MMM-yyyy", "dd-MM-yyyy" };
            bool isValidDate = DateTime.TryParseExact(filter.Value, formats, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime result);
            if (isValidDate)
            {
            ...
            else
            {
                switch (filter.Operator.ToLower())
                {
                    case "eq":
                        parameters.Add(filter.Value.Trim());
                        return string.Format("TRIM(Lower({0})) = '" + "{1}'",
                            filter.Field,
                            filter.Value.Trim().ToLower());
                    case "neq":
                        parameters.Add(filter.Value.Trim());
